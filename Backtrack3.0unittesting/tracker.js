...
$('#history').on('pageshow', function () {
	...
	$("#tracks_recorded").html("<strong>" + tracks_recorded + "</strong> path(s) recorded");
	...
	$("#history_tracklist li a").on('click', handleLoadHistory);

});

// When the user clicks a link to view track info, set/change the track_id attribute on the track_info page.
var handleLoadHistory = function() {
	sessionStorage.track_id = $(this).text();
};
...
$('#track_info').on('pageshow', function(){
	...
	var key = sessionStorage.track_id;
